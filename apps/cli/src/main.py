...
class MicrophoneProcessor:
    ...
    def _setup_wave_file(self):
        """配置并打开一个WAV文件用于录音。"""
        filename_base = (
            f"cli_recording_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.wav"
        )
        ...
        try:
            ...
            self.state.status_text = (
                f"录音将保存到: {os.path.basename(self.wave_filename)}"
            )
...
async def amain(args):
    ...
    try:
        ...
        uri = f"ws://{args.host}:{args.port}/ws/{args.client_id}"
